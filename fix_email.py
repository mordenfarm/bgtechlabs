...
`;
    ...
    // The most reliable way to copy rich HTML that pastes perfectly into Gmail:
    ...
    if (selection) {
      ...
      try {
        ...
        // Fallback to copying raw HTML code if execCommand fails
...
content = re.sub(
    r'const copyEmailDraft = async \(reg: any\) => \{[\s\S]*?setTimeout\(\(\) => setCopiedId\(null\), 3000\);\n\s*\}\n\s*\};',
    new_copy_func.strip(),
    content
)
